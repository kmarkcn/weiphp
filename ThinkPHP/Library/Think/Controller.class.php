...
abstract class Controller {
	...
	function initFollow($dao = false) {
		 ...
		 */
		//如果粉丝不存在则初始化粉丝信息
		addWeixinLog('-------------------开始判断用户是否存在-------------------');
		
		addWeixinLog('-------------------检查基本参数-------------------');
		addWeixinLog('token is', $map ['token']);
		addWeixinLog('openid is', $map ['openid']);
		
		addWeixinLog('-------------------查询用户数据-------------------');
		
		$usermap ['token'] = $map ['token'];
		$usermap ['openid'] = $map ['openid'];
		
		$user_exist = M ( 'follow' )->where ( $usermap )->find ();
		if(!$user_exist){
			$usermap ['subscribe_time'] = time ();
			...
			$User = new UserApi ();
			addWeixinLog('-------------------开始注册用户-------------------');
			$uid = $User->register ( $nickname, '123456', $email, '', $usermap ['openid'], $usermap ['token'] );
			
			addWeixinLog('uid', $uid);
			if ($uid > 0) {
			  $usermap ['id'] = $uid;
			   M ( 'follow' )->add ( $usermap );
			}
			addWeixinLog('-------------------注册用户成功-------------------');
		}
		addWeixinLog('-------------------流程完成-------------------');
